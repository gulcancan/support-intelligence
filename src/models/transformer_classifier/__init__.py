"""
Multi-task transformer classifier for support tickets.

Predicts four targets simultaneously:
  1. Category      (7 classes)  — e.g. Technical Issue, Bug Report
  2. Subcategory   (27 classes) — e.g. Configuration, Crash/Bug, API Extension
  3. Priority      (4 classes)  — critical, high, medium, low
  4. Sentiment     (6 classes)  — frustrated, angry, neutral, satisfied, confused, anxious

Architecture:
  text → Encoder → [CLS] (768-dim)
                        ↓
...
Loss: Multi-task focal loss (γ=2) with per-task weighting.
  Focal loss focuses training on hard examples, preventing the model
  from saturating on easy patterns (which caused F1=1.0 with cross-entropy).
...
TASK_DEFS = {
    ...
    "subcategory": {"column": "subcategory",        "weight": 1.5},  # harder task, higher weight
    "priority":    {"column": "priority",           "weight": 1.0},
    "sentiment":   {"column": "customer_sentiment", "weight": 0.8},  # noisier signal
}
...
class FocalLoss(nn.Module):
    ...
    With γ=0, this reduces to standard cross-entropy.
    With γ=2 (default), easy examples with p_t>0.9 get 100× less gradient.

    Args:
        gamma: focusing parameter (0=CE, 2=default focal)
        alpha: per-class weights (same as CE class weights), optional
...
class MultiTaskTicketDataset(Dataset):
    def __init__(self, texts, structured, labels_dict, tokenizer, max_length=512):
        ...
        Args:
            ...
            structured: np.ndarray (n, n_features)
            labels_dict: {"category": np.ndarray, "subcategory": np.ndarray, ...}
...
class MultiTaskTicketModel(nn.Module):
    """
    Shared encoder + per-task classification heads.

    The shared trunk after the encoder lets tasks share intermediate
    representations. Each head is a small 2-layer MLP.
    """

    def __init__(self, model_name, n_structured, task_n_classes, dropout=0.3, freeze_fraction=0.5):
        """
        Args:
            model_name: HuggingFace model name
            n_structured: number of structured features
            task_n_classes: dict of {task_name: n_classes}
            dropout: dropout rate
            freeze_fraction: fraction of encoder layers to freeze
        """
        super().__init__()
        ...
        all_layers = [n for n, _ in self.encoder.named_parameters() if "layer" in n or "layers" in n]
        n_freeze = int(len(all_layers) * freeze_fraction)
        frozen_prefixes = set()
        for name in all_layers[:n_freeze]:
            parts = name.split(".")
            for i, p in enumerate(parts):
                if p in ("layer", "layers") and i + 1 < len(parts):
                    frozen_prefixes.add(".".join(parts[: i + 2]))
                    break
        ...
        n_frozen = sum(1 for p in self.encoder.parameters() if not p.requires_grad)
        n_total = sum(1 for _ in self.encoder.parameters())
        logger.info(f"Encoder: {model_name}, hidden={hidden_size}, frozen={n_frozen}/{n_total}")
        ...
        logger.info(f"Task heads: {', '.join(f'{k}({v} classes)' for k, v in task_n_classes.items())}")
...
class TransformerTicketClassifier:
    ...
    Predicts category, subcategory, priority, and sentiment simultaneously.

    Usage:
        clf = TransformerTicketClassifier()
        metrics = clf.train(train_df, val_df)
        result = clf.predict(ticket_dict)
        # result.predicted_category, result.predicted_subcategory, etc.
    """
    ...
    def __init__(self, model_dir="./models/transformer", model_key=None, model_name=None,
        ...
        logger.info(f"Multi-task classifier: {self.model_name}, tasks={list(self.tasks.keys())}, focal_γ={focal_gamma}")
    ...
    def _encode_labels(self, df, fit=False) -> Dict[str, np.ndarray]:
        """Encode all task labels. Returns dict of {task_name: int_array}."""
        labels = {}
    ...
    def _build_focal_losses(self, y_train_dict) -> Dict[str, FocalLoss]:
        """Build focal loss per task with class-weight alpha."""
        losses = {}
    ...
    def train(self, train_df, val_df, epochs=5, batch_size=32, lr=2e-5):
        ...
        for ep in range(epochs):
            self.model.train()
            total_loss = 0
            task_losses_sum = {t: 0.0 for t in active_tasks}
            ...
            for bi, batch in enumerate(tr_dl):
                ...
                # Multi-task loss: weighted sum of focal losses
                ...
                for task_name in active_tasks:
                    ...
                    task_losses_sum[task_name] += task_loss.item()
            ...
            task_summary = " | ".join(
                f"{t}={val_metrics[t]:.4f}" for t in active_tasks
            )
            logger.info(f"Epoch {ep+1} loss={avg_loss:.4f} | {task_summary}")

            # Use mean F1 across all tasks for early stopping
        ...
        for task_name, f1 in final_metrics.items():
            logger.info(f"  {task_name}: F1={f1:.4f} ({len(self.label_encoders[task_name].classes_)} classes)")

        # Return category metrics for backward compatibility
        ...
        cat_metrics.task_f1_scores = final_metrics  # attach all task scores
        return cat_metrics

    def _validate_all(self, dl, y_val_dict) -> Dict[str, float]:
        """Compute weighted F1 per task."""
    ...
    def _predict_batch_internal(self, loader) -> Dict[str, np.ndarray]:
        """Return dict of {task_name: predicted_indices}."""
        self.model.eval()
    ...
    def predict(self, ticket) -> ClassificationResult:
        ...
        return ClassificationResult(
            ...
            model_name=f"multitask-{self.model_name.split('/')[-1]}",
            latency_ms=latency,
        )

    def predict_batch(self, df) -> pd.DataFrame:
        """Predict all tasks for a DataFrame. Returns DataFrame with prediction columns."""
    ...
    def save(self, path=None):
        ...
        logger.info(f"Saved multi-task model to {d}")
    ...
    def load(self, path=None):
        ...
        self.model.to(DEVICE)
        self.model.eval()
        logger.info(f"Loaded multi-task model from {d} ({self.model_name}, tasks={list(task_n_classes.keys())})")


# ── Backward-compatible aliases ─────────────────────────────────────────────
# For code that imports TicketDataset directly
